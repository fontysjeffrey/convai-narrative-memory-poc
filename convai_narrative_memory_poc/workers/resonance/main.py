...
from workers.common.utils import QDRANT_URL, QDRANT_COLLECTION, KAFKA_BOOTSTRAP, human_age
...
def search_anchors(client, query_vec, top_k=5):
    res = client.search(QDRANT_COLLECTION, query_vector=query_vec, limit=top_k, with_payload=True)
    return res
...
def decay_weight(stored_at_iso: str, now: dt.datetime) -> float:
    stored = dt.datetime.fromisoformat(stored_at_iso.replace("Z","+00:00")).replace(tzinfo=None)
...
def main():
    ...
    consumer = Consumer({'bootstrap.servers': KAFKA_BOOTSTRAP,
                         'group.id': 'resonance', 'auto.offset.reset':'earliest'})
    ...
    while True:
        ...
        if msg is None: 
            continue
        if msg.error():
            print(f"[resonance] error: {msg.error()}", file=sys.stderr); continue
        try:
            payload = json.loads(msg.value().decode('utf-8'))
            request_id = payload["request_id"]
            query = payload["query"]
            top_k = int(payload.get("top_k", 5))
            assumed_age = payload.get("assume_anchor_age")  # optional ISO 8601 duration, not used here
            now_iso = payload.get("now")
            now = dt.datetime.fromisoformat(now_iso.replace("Z","+00:00")).replace(tzinfo=None) if now_iso else dt.datetime.utcnow()
            ...
            for act, h in scored[:3]:
                pl = h.payload
                stored = dt.datetime.fromisoformat(pl["stored_at"].replace("Z","+00:00")).replace(tzinfo=None)
                beats.append({
                    "anchor_id": h.id,
                    "text": pl["text"],
