...
The core logic of this PoC is contained in the `poc.py` script. It simulates the creation of a memory and, using a Large Language Model (LLM) via Ollama, demonstrates how it's retold over different time intervals.

### Prerequisites

1.  **Python 3:** Make sure you have Python 3 installed.
2.  **Ollama:** This PoC requires [Ollama](https://ollama.com/) to be running locally.
3.  **An Ollama Model:** You need to have a model pulled. The script is configured to use `gpt-oss:latest`, but you can change it in `poc.py`. You can pull the model with:
    ```bash
...
1.  **Create a Virtual Environment:** It's best practice to use a virtual environment. We recommend using `uv`.
    ```bash
    python3 -m venv .venv
    ```
2.  **Install Dependencies:** Use `uv` to install the required packages from `requirements.txt`.
    ```bash
    uv pip install -r requirements.txt --python .venv/bin/python3
    ```
3.  **Run the Simulation:** Execute the script using the Python interpreter from your virtual environment.
    ```bash
    .venv/bin/python3 poc.py
    ```

### Expected Output

Running the script will simulate the creation of a backlog of memories with different ages and emotions (frustration, joy, surprise). It will then simulate a brief conversation by randomly recalling and retelling a few of these memories.

Because the memories are chosen randomly and the stories are generated by an LLM, the output will be different each time you run it. This demonstrates a more complex system where a character can draw from a pool of past experiences to share.
